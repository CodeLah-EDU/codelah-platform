...
function LessonCard({
  ...
  return (
    <section className="lesson-card" aria-labelledby={parent ? "parent-next" : "next-lesson"}>
...
function Feedback({ report, parent = false }: { report?: Report; parent?: boolean }) {
  return (
    <section className="paper-card feedback-card">
      ...
      <h2>{parent ? "This week, in Avery’s words and work." : "Your latest teacher update."}</h2>
      <p>
        {completed.title}
        <br />
        <span className="small-meta">
          {lessonDate(completed.start)} · {lessonTime(completed.start, completed.end)}
        </span>
      </p>
      {report ? (
        <>
  ...
  return (
    <section className="paper-card teacher-editor">
      <div className="section-heading editor-heading">
        ...
        <span className="status-pill neutral">{dirty ? "Draft changes" : "Published"}</span>
      </div>
      ...
      >
        {students.map((student) => (
          <label
            key={student.id}
            className={selected === student.id ? "student-choice selected" : "student-choice"}
          >
            <RadioGroupItem value={student.id} />
            <span className="avatar">{student.initials}</span>
            <span>
              {student.name}
              <small>{state.published[student.id] ? "Update published" : "No update yet"}</small>
            </span>
          </label>
        ))}
      </RadioGroup>
      ...
      >
        <fieldset className="attendance-field">
          ...
          <RadioGroup
            ...
            onValueChange={(value) => change({ attendance: value as Attendance })}
          >
    ...
    <div className="worksheet-reader">
      ...
      <ol className="challenge-list">
        <li>
          ...
          <p>
            Write three questions and their correct answers. Choose a topic you enjoy: animals,
            space, or your favourite game.
          </p>
        </li>
        <li>
          <h3>Check an answer</h3>
          <p>
            Read this sample. What changes when you call the function with “Mars”? What happens with
            “Venus”?
          </p>
          <pre>
            <code>{`let score = 0;\n\nfunction checkAnswer(answer) {\n  if (answer === "Mars") {\n    score += 1;\n    return "Correct!";\n  }\n  return "Try again.";\n}`}</code>
          </pre>
          <p className="small-meta">Reading example only · Code does not run in this preview.</p>
        </li>
        <li>
          <h3>Explain your next step</h3>
          <p>
            How would you stop someone earning points by answering the same question twice? Bring
            your idea to class.
          </p>
        </li>
      </ol>
      <div className="worksheet-end">
        ...
        <p>
          ...
          <br />
          What an event handler does, when a score changes, and how to test an incorrect answer.
        </p>
      </div>
    </div>
  );
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const tool: Tool = {
      ...
      inputSchema: {
        type: "object",
        properties: { role: { type: "string", enum: ["student", "teacher", "parent"] } },
        required: ["role"],
    ...
    try {
      void Promise.resolve(registry.registerTool(tool, { signal: lifecycle.signal })).catch(
        (error) => console.warn("Optional demo tool unavailable", error),
      );
    } catch (error) {
  ...
  return (
    <div className="studio">
      ...
      <Tabs value={role} onValueChange={(value) => setRole(value as Role)} className="portal-tabs">
          ...
          <TabsContent value="parent">
            ...
            <div className="lower-grid">
              ...
              <aside className="paper-card">
                ...
                <p>
                  Ask: “What happens when I choose the wrong answer?” Give Avery time to explain and
                  test the idea.
                </p>
      ...
      >
        <DialogContent className="lesson-dialog">
          <DialogTitle className="dialog-title">
            {modal === "classroom" ? "Your classroom, inside CodeLah." : "Quiz Lab worksheet"}
          </DialogTitle>
          <DialogDescription>
            {modal === "classroom"
